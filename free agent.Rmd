...
avg_clusters = hitting_free_agents_stats %>% plot_ly(type = 'scatter', x= ~as.numeric(avg), y= ~est_ba, marker = list(size = ~plate_appearances, sizeref = 5, sizemode = 'area'), color = ~as.factor(xba_cluster), text = ~player_full_name, hovertemplate = paste('<b>%{text}</b>
      %{yaxis.title.text}: %{y},
      %{xaxis.title.text}: %{x}'))
avg_clusters
```
